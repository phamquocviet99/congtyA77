...
function CategoryActivityAdmin() {
  const loadPage = "";
  ...
  async function DeleteCategory(id) {
    try {
      setLoading(true);
      await categoryActivityApi.remove(id);
      setPageLoad(id);
      setLoading(false);
    } catch (error) {
      setError(true);
  ...
  async function addNewCategory() {
    try {
      setLoading(true);
      const response = await categoryActivityApi.create(category);
      setLoading(false);
    } catch (error) {
      setError(true);
    }
  ...
  async function updateCategory() {
    try {
      setLoading(true);
      const response = await categoryActivityApi.update(idUpdate, category);
      setLoading(false);
    } catch (error) {
      setError(true);
    }
              ...
              <div className="table-title">
                <div className="row">
                  <div className="col-sm-6">
                    <h2 >
                      Quản lý <b>Danh mục lĩnh vực hoạt động</b>
                    </h2>
                  </div>
                  <div className="col-sm-6">
                    <button
...
export default CategoryActivityAdmin;
